...
# Assuming AutoLambda is defined somewhere in your project structure.
from your_module.autolambda import AutoLambda
...
def main(cfg: DictConfig) -> None:
    ...
    # Initialize AutoLambda
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = # Instantiate your model here
    train_tasks = # Define your training tasks here
    pri_tasks = # Define your primary tasks here
    auto_lambda = AutoLambda(model, device, train_tasks, pri_tasks, weight_init=0.1)

    # Run training jobs with multiple seeds (sequentially)
    for seed in range(start_seed, start_seed + cfg.framework.seeds):
        ...
        mp.spawn(run_seed_fn.run_seed,
                 args=(cfg,
                       ...
                       seed,
                       world_size,
                       auto_lambda,),  # Pass AutoLambda instance to the function
                 nprocs=world_size,
                 join=True)
